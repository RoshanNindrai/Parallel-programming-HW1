...
/**
 * This function is used to compute the INS scores in Parallel
 * @param _number_of_students Number of students
 * @param _number_of_exams	  Number of exams taken by each student
 * 
 */
public void computeINS(final int _number_of_students,final int _number_of_exams) throws Exception{

new ParallelTeam().execute (new ParallelRegion()
{
	public void run() throws Exception
	{
	execute (0,_number_of_students,new IntegerForLoop()
	{
		float temp_score;
		float square;
		...
		public void start()
		{
			...
			square=0;
			// Initializing the INS array
			student_INS=new float[_number_of_students];
			tempstudent_INS=new float[_number_of_students];
		}
		...
		for(int j=0;j<_number_of_exams;j++){
			...
			square=matrix_score[i][j]*matrix_score[i][j];
			temp_score=temp_score+(square);
		}
